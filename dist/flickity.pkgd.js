...
function Cell( elem, index, parent ) {
  this.element = elem;
  this.index = index;
  this.parent = parent;
...
Cell.prototype.setPosition = function( x ) {
  ...
  this.element.style['z-index'] = 0 - this.index;

    this.parent.on('moveX', this.applyProximityEffect.bind(this));
};
...
Cell.prototype.applyProximityEffect = function() {
    var proximity = Math.abs(this.target - Math.abs(this.parent.x));

    proximity *= 0.1;

//    console.log(Math.round(proximity) / 1);

    var scale = Math.round((100 - Math.round(proximity) / 1)) / 100;

    if (scale >= 0.8 && scale <= 1) {
        this.element.style.transform = 'scale(' + scale + ')';
        // this.element.style['opacity'] = scale;
    }

    // Need to set z-index to top when moving in
    // and back to normal when moving out.
};
...
proto.applyProximityGraph = function() {
    this.dispatchEvent('moveX');
};
